...
#include <random>
  ...
  #define ROS2_TYPE_NAME "test_msgs/Arrays"
#else
  #include <test_msgs/msg/bounded_array_nested.hpp>
  #define ROS2_TYPE_NAME "test_msgs/BoundedArrayNested"
#endif // RCLCPP__QOS_HPP_

template<typename Ros2Primitives>
Ros2Primitives generate_random_primitives(const std::size_t seed)
{
  std::mt19937 rng;
...
template<typename Ros2Primitives>
xtypes::DynamicData generate_random_primitives_msg(const std::size_t seed, const xtypes::DynamicType& type)
{
...
#define SOSS_SET_FIELD(field_name) \
  soss::Convert<typename Ros2Primitives::_ ## field_name ## _type>::to_xtype_field(ros2_primitives.field_name, xtypes_primitives[#field_name]); \
...
Ros2Array generate_random_array(std::size_t N)
{
  Ros2Array msg;
...
template<typename Ros2Array, typename Ros2Primitives>
void test(rclcpp::Node::SharedPtr& ros2, rclcpp::executors::SingleThreadedExecutor& executor) {
  ...
  for(std::size_t i = 0; i < N; ++i)
  {
    ...
    #define SOSS_REQUIRE_AND_COMPARE(field_name) { \
      ...
      soss::Convert<typename Ros2Primitives::_ ## field_name ## _type>::from_xtype_field(xtypes_primitives[#field_name], ros2_field); \
...
TEST_CASE("Transmit and receive all test messages", "[ros2]")
{
  ...
  {
#ifdef RCLCPP__QOS_HPP_
    test<test_msgs::msg::Arrays, test_msgs::msg::BasicTypes>(ros2, executor);
#else
    test<test_msgs::msg::BoundedArrayNested, test_msgs::msg::Primitives>(ros2, executor);
#endif // RCLCPP__QOS_HPP_
  }
