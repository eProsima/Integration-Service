...
using MockSubscriptionCallback = std::function<void(const dds::core::xtypes::DynamicData&)>;
...
std::shared_future<dds::core::xtypes::DynamicData> SOSS_MOCK_API request(
    const std::string& topic,
    const dds::core::xtypes::DynamicData& request_msg,
    std::chrono::nanoseconds retry = std::chrono::seconds(0));


using MockServiceCallback = std::function<dds::core::xtypes::DynamicData(const dds::core::xtypes::DynamicData& request)>;
