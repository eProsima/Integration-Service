    ...
    return soss_message.for_each([&](xtypes::DynamicData::WritableNode& soss_node)
                   {
                       ...
                       switch (soss_node.type().kind())
                       {
                           ...
                           case xtypes::TypeKind::STRING_TYPE:
                               soss_node.data().value(access_json_value(soss_node,
                           ...
                           case xtypes::TypeKind::FLOAT_32_TYPE:
                               soss_node.data().value(get_json_float<float>(access_json_value(soss_node,
                               json_stack.top())));
                               break;
                           case xtypes::TypeKind::FLOAT_64_TYPE:
                               soss_node.data().value(get_json_float<double>(access_json_value(soss_node,
                               json_stack.top())));
                               break;
                           default:
                               throw false;
